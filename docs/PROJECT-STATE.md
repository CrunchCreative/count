...
- `docs/phases/phase-3-brief.md` / `phase-3-summary.md` — Note Pad + Fixtures list + route stub
- `docs/phases/phase-3.5-brief.md` / `phase-3.5-summary.md` — persistent header + Note Pad bar regression fix + sheet close gestures + filter chip rendering attempt + referee row
- `docs/phases/phase-3.5-debug-notes.md` — diagnostic run for the Note Pad bar visibility/tappability bug (gotcha #10)
- `docs/phases/phase-3.6-brief.md` / `phase-3.6-summary.md` — bigger logo, transparent header at rest, scroll-driven blur
- `docs/phases/phase-3.6.1-brief.md` / `phase-3.6.1-summary.md` — logo alignment + scroll reset on tab focus
- `docs/phases/phase-3.6.2-brief.md` / `phase-3.6.2-summary.md` — filter chip rendering attempt (incomplete — fix landed iteratively after device testing; see workflow notes)
...
8. **Two type weights only**: 400 and 500. **Borders default to `StyleSheet.hairlineWidth`** but see gotcha #11 — hairlines below ~0.20 alpha are functionally invisible on iPhone dark backgrounds; use 1pt at ~0.14 alpha for any border that needs to read clearly. **No emoji anywhere.** SVG icons only.
...
13. **Persistent app header is mounted at the `(tabs)` layout level** (Phase 3.5+). Logo + profile icon. Hides on the fixture detail route by virtue of being scoped to the tab group. Background animates from transparent to blurred-glass as the user scrolls, driven by a shared `ScrollContext.scrollY` Animated.Value written by each tab screen's `Animated.ScrollView.onScroll`. Scroll resets to 0 on every tab focus via `useFocusEffect`.
...
3. **Claude Code in the Codespace terminal** (`npx -y @anthropic-ai/claude-code`, or `claude` after alias setup) executes the brief autonomously, reading the brief, the design source, and the existing scaffold; writing the code directly into the repo; running typechecks and bundle verifications.
...
**Brief-writing discipline (lesson from Phase 3.5/3.6.2):** When porting from the design source, cite CSS by selector AND line number, not by vibe. Phase 3.5's filter chip rendering bug was traceable to the brief saying "match `.date-seg` from styles.css" without specifying the structural relationship between `.date-strip` (the container) and `.date-seg` (the segments inside). Claude Code built the segments as standalone chips with their own borders. The fix required restructuring the JSX to wrap segments in a strip container. **Future briefs must reproduce the relevant CSS values verbatim and call out the parent/child DOM relationship explicitly.**
...
- **Phase 3** — Note Pad subsystem (provider, hook, sheet, leg row — all new; Phase 2A's `NotePadBar` was visual-only chrome). Full `FixturesList` screen (date strip, comp filter, league-grouped fixture cards with addable Safe pills). `/fixture/[id]` route stub (header + H1 + 4-tab strip + placeholder; no tab content yet). First *interactive* addable mechanic shipped — tapping a `+` Safe pill toggles the leg in the Note Pad with visual feedback. `SafePill` extended additively (new `leg?: Leg` prop, existing `addable?: boolean` preserved for visual-only callers). Verified on real iPhone via Expo Go.
- **Phase 3.5** — Persistent header subsystem (`AppHeader` — logo + profile icon, mounted at `(tabs)/_layout.tsx`, hides on fixture detail). Note Pad bar visibility regression diagnosed and resolved (multi-attempt — see gotcha #10). Sheet close gestures: tap drag-handle + drag-down via `PanResponder`. Filter chip rendering attempt (incomplete — full fix landed iteratively across 3.6.2 and follow-ups). Referee row on `FixtureListCard` (between teams row and top-angle inset — extends `FixtureListItem` with `referee: { name, cardsPerMatch, redsPerMatch }`). New icons: `card-yellow`, `card-red`. Verified on real iPhone via Expo Go.
- **Phase 3.6** — Logo +30% (`LOGO_HEIGHT` 32 → 42, `APP_HEADER_CONTENT_HEIGHT` 56 → 72). Header background extracted to an `Animated.View` whose opacity interpolates `scrollY` 0–60 → 0–1 with `extrapolate: 'clamp'`. New `ScrollContext` providing a shared `scrollY: Animated.Value` written by each tab screen's `Animated.ScrollView.onScroll`. Foreground row (logo + profile icon) stays crisp regardless of scroll. Verified on real iPhone via Expo Go. Follow-up: gradient overlay opacity dropped from `rgba(8,9,11,0.92/0.78)` to `rgba(255,255,255,0.04/0.015)` so the BlurView does the work and the surface reads as translucent rather than solid dark.
- **Phase 3.6.1** — Logo `marginLeft` -4 → -12 (alignment with screen-edge H1s). `ScrollContext` extended with `resetScroll()` + new `useResetScroll` hook. `FixturesList` converted to `forwardRef<FixturesListHandle, FixturesListProps>` exposing `scrollToTop()`. Each tab screen's ScrollView wired with `useFocusEffect` to scroll to top + reset scrollY on tab focus. Verified on real iPhone via Expo Go.
- **Phase 3.6.2** — Filter chip rendering fully fixed. Restructured the date strip as a single bordered container (`.date-strip` from source) with four transparent `DateChip` segments inside (`.date-seg`, `flex: 1` each, active = teal-tinted background + teal label + iOS shadow glow). `CompChip` height matched to `GlassSelect` trigger (`paddingVertical: 9`); active state gets an iOS amber shadow glow. `GlassSelect` trigger rebuilt with a single static merged style. All chip styles passed as single static objects, not arrays (gotcha #10). Verified on real iPhone via Expo Go.
...
10. **`Pressable` drops layout/visual properties from inline style objects inside style arrays (Phase 3.5 + 3.6.2).** Distinct from gotchas #1 and #9 but the same family. Symptoms seen: `NotePadBar` rendering with `height: 0.7px` despite `height: 52` in the array; `DateChip` and `CompChip` showing no border / no `backgroundColor` despite both being in their style arrays. Fix in every case: pass a SINGLE merged static `ViewStyle` object to the Pressable's `style` prop — never `style={({ pressed }) => [obj, obj, obj]}` when the array contains layout or visual styles. Pattern that works:
    ```ts
    const mergedActiveStyle: ViewStyle = { ...baseStyle, ...activeStyle };
    return <Pressable style={active ? mergedActiveStyle : mergedInactiveStyle}>...
    ```
    For press feedback, use a child `<View>` overlay with the press-tinted style, or fall back to `opacity` on the Pressable itself (which is one of the few properties that survives the array form). When in doubt, merge — don't array.

11. **Hairline borders below ~0.20 alpha don't render visibly on iPhone dark backgrounds (Phase 3.5+).** `colors.border.default` at `StyleSheet.hairlineWidth` × ~0.10 alpha is functionally invisible against the page background (`#08090B`). The Phase 3.5 chip-rendering fix that bumped to `colors.border.strong` (~0.10 alpha) wasn't enough — borders still vanished. The visibility threshold for hairlines is around 0.20 alpha on dark; below that they alias to nothing. Solutions: (a) use `borderWidth: 1` (not hairline) at lower alpha if subtlety matters, e.g. `rgba(255,255,255,0.14)`; (b) keep hairline but bump alpha to ~0.22+. Don't trust the source CSS's `0.5px solid var(--b-def)` to translate directly — browsers anti-alias differently than RN. Verify visibility on real device, not just simulator.
...
- Android device pass deferred from Phase 2A, 2B, 3, and all 3.5/3.6 sub-phases — every phase verified on iPhone only. Android verification should happen before Phase 4 (Team/Player stats grid is the most layout-sensitive screen and Android quirks will bite hardest there) or as part of Phase 12 polish at the latest.
...
- Whether the persistent header should hide the back/share/bookmark row on fixture detail in favour of a unified header that swaps icon sets per route, or stay as-is with the route's own header. Currently stays as-is. Revisit if fixture detail starts feeling visually disjointed from the tab screens.
...
- Claude Code launched via `npx -y @anthropic-ai/claude-code` (or the `claude` alias in `~/.bashrc`).
- Expo Go on iPhone, scanning QR from `npx expo start --tunnel`. Tunnel via ngrok sometimes flaky — fall back to `npx expo start` + VS Code port-forward 8081 if needed.