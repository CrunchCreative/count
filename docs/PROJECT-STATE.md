...
10. **`Pressable` drops layout properties from inline style objects inside style arrays (Phase 3.5). Distinct from gotchas #1 and #9. When a Pressable's style prop receives an array containing an inline-object style with layout properties (position, height, width, top/left/right/bottom), RN can drop those properties at measure time. Symptom in Phase 3.5: NotePadBar rendered with height: 0.7px despite height: 52 being in the style array. Fix: extract the full positioning style to a single static object passed directly to style (no array). Acceptable pattern: style={positioningStyle} where positioningStyle: ViewStyle = { position: 'absolute', height: 52, ... }. Avoid pattern: style={[{ position: 'absolute', height: 52 }, otherStyle]}. When in doubt for any Pressable, build one merged static style object instead of an array.
